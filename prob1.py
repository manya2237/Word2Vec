...
cle = [w for w in tokens if w not in s and len(w) > 2]

print("Total tokens after cleaning:", len(cle))
...
print("\n--- DATASET STATISTICS ---")
print("Documents: 3")
print("Total Tokens:", len(cle))
print("Vocabulary Size:", len(set(cle)))
...
def genera(tok, wi):
    ...
    for i, word in enumerate(tok):
        context = []
        for j in range(i - wi, i + wi + 1):
            if j != i and 0 <= j < len(tok):
                context.append(tok[j])
        data.append((word, context))
    return data

training_data = genera(cle, window_size)


# Step 3: Initialize weights
W1 = np.random.rand(v_size, embedding_dim)
W2 = np.random.rand(embedding_dim, v_size)
...
def trai(epochs=3, lr=0.01, neg=5):
    global W1, W2
    ...
    for epoch in range(epochs):
        ...
        for target, context_words in training_data:
            ...
            for c in context_words:
                ...
                # Negative samples
                for _ in range(neg):
                    neg_idx = random.randint(0, v_size - 1)

                    score = np.dot(W1[target_idx], W2[:, neg_idx])
                    pr = sigmoid(score)

                    loss -= np.log(1 - pr + 1e-9)
                    grad = pr
...
def most_similar(word, top_n=5):
    ...
    for w in vocab:
        if w != word:
            sim = cosi(target_vec, W1[word_to_index[w]])
            simi.append((w, sim))
...
def print_neighbors(word):
    ...
    try:
        neigh = most_similar(word, top_n=5)
...
for wwww in words_to_check:
    print_neighbors(wwww)
...
def analogy(word_a, word_b, word_c, top_n=3):
    ...
    for w in vocab:
        if w not in [word_a, word_b, word_c]:
            score = cosi(vec, W1[word_to_index[w]])
